	...
	function course_price(){
		...
		if( class_exists('LifterLMS') && ! llms_is_user_enrolled( get_current_user_id(), $post_id ) && class_exists('LLMS_Integration_WooCommerce') ){
			...
			if ( $id ) {
				...
				$minimum_price = esc_html__( 'Course Price:' , 'michigan' ) . ' ' . get_lifterlms_currency_symbol() . $price ;
			}
			...
			if ( ! array_values( $posts ) ){
				$minimum_price = esc_html__( 'No access plans exist.', 'michigan' );
			}
...
function michigan_webnus_maintenance_mode() {
	$michigan_webnus_options = michigan_webnus_options();
	$is_maintenance = (isset($michigan_webnus_options['michigan_webnus_maintenance_mode']) && $michigan_webnus_options['michigan_webnus_maintenance_mode'] ) ? $michigan_webnus_options['michigan_webnus_maintenance_mode'] : '';
	$maintenance_page = (isset($michigan_webnus_options['michigan_webnus_maintenance_page']) && $michigan_webnus_options['michigan_webnus_maintenance_page'] ) ? $michigan_webnus_options['michigan_webnus_maintenance_page'] : '';
...
function michigan_webnus_excerpt($limit) {
	global $post;
	$post_id = get_the_ID();
	$w_post = get_post( $post_id ); 
	$w_excerpt = !empty($w_post->post_excerpt) ? $w_post->post_excerpt : $w_post->post_content;
	$excerpt = explode(' ', $w_excerpt, $limit);
...
// Ticker
	$michigan_webnus_options['michigan_webnus_news_ticker'] = isset( $michigan_webnus_options['michigan_webnus_news_ticker'] ) ? $michigan_webnus_options['michigan_webnus_news_ticker'] : '';
	if($michigan_webnus_options['michigan_webnus_news_ticker'])
...
function michigan_webnus_page_background_override(){
	...
	if($wrap_color){
		$out .= '} #wrap{background:'.$wrap_color.' !important;';
		if ( $bgimage ) {
			$out .= 'background: none;';
		}
	}
...
function michigan_webnus_wphead_action(){
	...
	$michigan_webnus_options['michigan_webnus_space_before_head'] = isset($michigan_webnus_options['michigan_webnus_space_before_head']) ? $michigan_webnus_options['michigan_webnus_space_before_head'] : '';
	$michigan_webnus_options['michigan_webnus_typekit_id'] = isset($michigan_webnus_options['michigan_webnus_typekit_id']) ? $michigan_webnus_options['michigan_webnus_typekit_id'] : '';
